...
function initWhatMattersSlider() {
	...
	if (!root) return;
	const tabsRoot = document.querySelector("[data-fls-what-matters-tabs]");
...
//#endregion
//#region src/components/pages/index/resources-slider.js
function initResourcesSlider() {
	const root = document.querySelector("[data-fls-resources-slider]");
	if (!root) return;
	const prevButtons = document.querySelectorAll("[data-fls-resources-prev]");
	const nextButtons = document.querySelectorAll("[data-fls-resources-next]");
	...
	const swiper = new Swiper(root, {
		...
		on: {
			init: updateArrowsState,
			slideChange: updateArrowsState
		}
	});
	prevButtons.forEach((button) => {
		button.addEventListener("click", () => swiper.slidePrev());
...
function initCustomerSuccessSlider() {
	...
	function syncNav(activeSwiper) {
		...
		else root.insertBefore(clone, root.firstElementChild);
	}
	const swiper = new Swiper(root, {
		modules: [Navigation],
		observer: true,
	...
	root.addEventListener("click", (event) => {
		const prevButton = event.target.closest("[data-fls-customer-success-prev]");
		const nextButton = event.target.closest("[data-fls-customer-success-next]");
		const logoButton = event.target.closest(".customer-success__logo");
		if (prevButton) {
			swiper.slidePrev();
			return;
		}
...
document.querySelector("[data-fls-customer-success-slider]") && window.addEventListener("load", initCustomerSuccessSlider);
//#endregion
